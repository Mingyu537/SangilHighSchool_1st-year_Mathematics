...
st.set_page_config(page_title="Neo 3D Design Generator", page_icon="3D", layout="wide")
st.title("Neo 3D Design Generator")
st.caption("Streamlit wrapper for generating a holographic glass-style Blender render.")

with st.sidebar:
    st.header("Render settings")
    size = st.slider("Image size", min_value=512, max_value=2400, value=1200, step=128)
    samples = st.select_slider("Cycles samples", options=[32, 64, 96, 128, 192, 256], value=96)
    output_name = st.text_input("Output filename", value="neo_3d_design.png")
    render_clicked = st.button("Render with Blender", type="primary")

safe_output_name = Path(output_name).name or "neo_3d_design.png"
command = (
    "blender --background --python render_neo_3d_design.py -- "
    f"--output {safe_output_name} --size {size} --samples {samples}"
)
...
with left:
    ...
    st.write(
        "This app can render locally when Blender is installed on the machine running Streamlit. "
        "On Streamlit Cloud, install Blender first or download the script and render it on a local computer."
    )

    if render_clicked:
        blender_path = shutil.which("blender")
        if blender_path is None:
            st.error("Blender was not found in PATH. Install Blender or run the command above locally.")
        else:
            with tempfile.TemporaryDirectory(prefix="neo3d_") as temp_dir:
                temp_path = Path(temp_dir)
                script_path = temp_path / "render_neo_3d_design.py"
                ...
                if result.returncode == 0 and output_path.exists():
                    ...
                    st.error("Blender render failed.")
                    st.code((result.stderr or result.stdout)[-4000:], language="text")

with right:
    st.subheader("Blender script")
    st.download_button(
        "Download render script",
        data=BLENDER_SCRIPT,
        file_name="render_neo_3d_design.py",
        mime="text/x-python",
    )
    with st.expander("View embedded Python code", expanded=False):
